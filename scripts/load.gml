sprite_change_offset("idle", 24, 52);
sprite_change_offset("idle_quack", 24, 52);
sprite_change_offset("hurt", 24, 52);
sprite_change_offset("hurt2", 24, 52);
sprite_change_offset("hurt_quack", 24, 52);
sprite_change_offset("crouch", 24, 52);
sprite_change_offset("crouch_quack", 24, 52);
sprite_change_offset("slide", 24, 52);
sprite_change_offset("slide_quack", 24, 52);
sprite_change_offset("walk", 24, 52);
sprite_change_offset("walk_quack", 24, 52);
sprite_change_offset("jump", 24, 52);
sprite_change_offset("jump_quack", 24, 52);

sprite_change_offset("hurtbox", 24, 52);
sprite_change_offset("crouchbox", 24, 52);
sprite_change_offset("airbox", 24, 52);
sprite_change_offset("hurt_hurtbox", 24, 52);

//right wing
sprite_change_offset("wing_idle", 24, 52);
sprite_change_offset("wing_crouch", 24, 52);
sprite_change_offset("wing_slide", 24, 52);
sprite_change_offset("wing_walk", 24, 52);
sprite_change_offset("wing_jump", 24, 52);
sprite_change_offset("wing_glide", 24, 52);

//left wing
sprite_change_offset("wing_left_jump", 24, 52);
sprite_change_offset("wing_left_glide", 24, 52);

sprite_change_offset("plat", 64, 94);

//equipment
sprite_change_offset("equipment_jetpack", 15, 15);
sprite_change_offset("equipment_walljump_boots_stand", 24, 52);
sprite_change_offset("equipment_walljump_boots_crouch", 24, 52);
sprite_change_offset("equipment_helmet", 14, 15);
sprite_change_offset("equipment_chestplate", 12, 11);

//vfx
sprite_change_offset("equipment_jetpack_smoke", 16, 18);

//hats sprites
sprite_change_offset("hat_uglies", 32, 32);